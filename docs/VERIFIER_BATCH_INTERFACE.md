...
Fixture-file artifact는 하위호환 필드와 정규화 필드를 함께 제공한다. 새 consumer는 정규화 필드를
우선 사용한다.
...
```json
{
  ...
  "resources": {
    "vocabulary": {
      ...
      "baseline_path": "/vocabs/7/words/",
      "read_path": "/vocabs/6/words/",
      "safe_mutation": {
        "method": "PUT",
        "path": "/vocabs/6/description/",
        "observe_path": "/vocabs/?owner_id=4",
...
기존 `victim_vocabulary`/`attacker_vocabulary` 분리 필드는 c2-04 verifier 하위호환을 위해 유지한다.
`safe_mutation`은 되돌릴 수 있는 benign marker 변경만 허용하고, destructive action은 fixture에 넣지 않는다.
...
- fixture-file이면 `candidate_from_fixture(run_id, fixture_path)`를 사용해 baseline/attack path와
  marker를 채운다. bridge 확장은 `resources.<kind>.attacker_id/victim_id` 정규화 쌍을 우선 사용하고,
  없으면 기존 c2-04 분리 필드로 fallback한다.
...
Patch diff 경로 기준은 **`catalog.source_root_for(target_id)` 기준 상대 경로**로 고정한다. 예를 들어
manifest `source_dir`가 `.vibecutter/targets/sources/26s-w1-c1-05/backend`인 target에서
`UserProfileController.java`가 `backend/src/main/...`에 있다면 diff 헤더는
`+++ b/src/main/.../UserProfileController.java`여야 한다. `vc_apply_patch`는 target Git repo 루트
worktree를 만들고, 그 안의 `manifest.source_dir` 대응 디렉터리(`catalog.run_source_root_for`)에서
`git apply`를 수행한다.

현재 `vc_map_routes`/`vc_map_roles`/`vc_index_code` 개별 mapping tool은 스텁이지만,
`vc_scan_access_control`은 P1이 배선해 `READY → MAPPING → CANDIDATE_SCAN` 전이를 한 번에 처리한다.
P3가 제공한 `surface.candidates.candidates_for_target(run_id, provisioning, source_root)`이 이 배선의
단일 후보 진입점이다.
