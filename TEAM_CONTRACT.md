...
### 3A-6. 패치 전달·보존 (P1) — ⚠️ 실제 결함

`reset_run()`이 Compose 정리 후 **worktree를 제거한다**(`runtime/target_service.py`). 사용자의 목표는 **"패치를 받는 것"**인데 지금은 정리와 함께 사라진다.

- [ ] **`vc_export_patch(run_id)` 신규** — diff를 `.vibecutter/runs/<run_id>/security-fix.patch`로 보존. **reset보다 먼저** 호출된다
- driver는 reset 전에 반드시 export를 거친다
- **원본 branch 반영은 우리가 하지 않는다.** 사용자가 `git apply`하는 별도 행위 — 기획서 10.1 "원본 미변경" 유지

### 3A-7. 승인 후 재개 흐름 (P1)

`RunState.WAITING_APPROVAL`은 이미 상태 기계에 있다(`PATCH_PROPOSED → WAITING_APPROVAL → PATCH_APPLIED`). 그런데 driver가 `confirmed=True`를 자동으로 넘겨 **그 상태를 스쳐 지난다.**

```
vc_audit_target(mode="propose")  → PATCH_PROPOSED 에서 정지
  ↓ Host가 diff를 사용자에게 표시
vc_apply_patch(patch_id, confirmed=True)  → WAITING_APPROVAL → PATCH_APPLIED
  ↓
vc_build_and_test → vc_replay_attack → vc_validate_regression → FIXED
  ↓
