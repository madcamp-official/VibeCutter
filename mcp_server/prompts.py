"""MCP Prompts (6.5절): `audit_local_target` + 단계별 4종.
...
`audit_local_target`은 전체 감사 흐름 하나를 안내하고, 나머지 4종(6.5절 표)은 그 흐름의
단계별 조각을 별도로 진입할 수 있게 한다: `verify_candidate`(특정 후보 최소 재현),
...
3. 생성된 patch diff를 **사용자에게 그대로 보여주고 명시적 승인을 받는다.**
4. 승인을 받은 뒤에만 vc_apply_patch(patch_id, confirmed=True)를 호출한다 — confirmed 없이는
   tool이 코드 레벨에서 거부한다. 절대 임의로 적용하지 않는다.
...
6게이트로 재검증한다. vc_apply_patch 이후에 쓴다:
...
4. verdict가 FIXED면 완료다. RETRY면 repair_verified_finding 프롬프트로 돌아가 다시
   패치한다 — 단 **재시도 상한 3회**를 넘기면 자동으로 human review로 넘어가니 강행하지 않는다.
5. verdict가 HUMAN_REVIEW면 자동 수리 범위를 벗어난 것이다 — 사용자에게 증거와 함께 보고한다.
