...
**질문 원칙(C3, SKILL.md "질문 원칙"과 동일)**: 사용자에게 묻는 모든 질문은 예/아니오 또는
보기 선택만이다 — 자유 서술형으로 값을 받지 않는다. agent가 레포에서 스스로 알아낼 수
있는 건 묻지 않는다: `vc_scaffold_manifest`가 감지한 값 중 확신 있는 것(`warnings`에 없는
것)은 계획에 그대로 넣어 **보고**하고, 확신이 낮은 것(`warnings`에 남은 것, 예: 포트 추측)만
"~로 봤는데 맞나요?"로 **확인**만 한다 — "포트가 몇 번이에요?"처럼 값 자체를 사용자에게
되묻지 않는다.
"""
...
1. vc_scaffold_manifest(source_path)로 build/start/stop/reset·포트·adapter 초안과 "어느
   ...
   `warnings`를 쉬운 말로 설명한다. 부족한 값은 **가능하면 보기 선택으로** 묻는다(예: "포트가
   3000/8000/8080 중 하나인가요, 다른 번호인가요?") — 정말 알 방법이 없을 때만 자유롭게
   묻는다. 추측으로 조용히 채우지 않는다.
2. **raw argv를 그대로 보여주지 말고, 감지한 내용을 앱·데이터 말로 요약해 승인을 구한다**
   (질문 원칙, C3). 확신 있는 값(`evidence`는 있고 `warnings`엔 없는 값)은 그냥 계획에
   포함해 보고하고, 확신이 낮은 값(`warnings`에 남은 것, 예: 포트 추측)만 "~로 봤는데
   맞나요?"로 짚어 확인한다. 마지막은 하나의 예/아니오 질문으로 모은다: "앱을 검사하려면
   앱을 먼저 켜야 해요 — 평소 시작 명령을 그대로 실행해도 될까요? [네/아니오] (자세히
   보기)". "자세히 보기"를 요청할 때만 `manifest.commands`의 실제 argv를 보여준다.
