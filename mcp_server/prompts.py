...
4. 각 candidate를 vc_verify_access_control(읽기 IDOR) / vc_verify_mutation_access_control
   (쓰기 IDOR — 공격자가 피해자 자원을 실제로 바꿨는지 before/after로 판정) /
   vc_verify_injection / vc_verify_xss 중 맞는 것으로 approved=True를 명시해 실제 재현
   검증한다.
