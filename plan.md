...
- [x] **L-4. LLM endpoint 티어 체인 배선** (Phase LLM L-1 확장) — **완료**: `model/endpoints.py` 신규(env 해석 + 티어 정책, 순수 함수 `resolve_tiers`로 네트워크 없이 테스트). primary=내부망→외부망 `qwen3-235b`(timeout 600s, 7.7 tok/s라 긴 응답이 정상), fallback=기존 7B. 앞 tier가 답을 못 주거나 timeout이면 다음으로 자동 승계(`make_chained_chat_fn`). 빈 응답도 실패로 처리 — rerank 파서가 빈 텍스트를 항등 순열로 삼켜 fallback 기회를 잃기 때문. 구성 시 1회 `GET /health`(인증 불필요) 3초 probe로 죽은 tier 제외 → 오프라인/CI에서 600초 멈춤 방지, 전부 죽었으면 None=휴리스틱. qwen3의 `<think>` 블록 제거 추가(사고 과정의 숫자가 rerank 순열을 오염시킴). 기존 `VIBECUTTER_MODEL_ENDPOINT`/`_NAME`은 **fallback(7B) 자리**로 하위호환 유지. 테스트 12건 신규 + 기존 8건 + wiring 3건 갱신.
  ...
  - ~~**Phase LLM L-2(모델 바인딩 팀 결정) 해소**: 외부 API 전환으로 "GPU 서버 위에서 loop을 돌릴지" 문제가 사라졌다.~~ **❌ 틀렸다 — R-5 참조.** 235B가 내부망에서만 닿아서(랩탑·P2 로컬 모두 timeout) "어디서 loop을 돌릴지"가 형태만 바꿔 그대로 남았다. env로 endpoint만 받는 구조는 그대로지만, **닿는 곳에서 돌려야 한다**는 제약이 새로 생겼다.
...
- [x] **R-1. 코드 컨텍스트를 프롬프트에 싣는 경로** — **완료**: `scanners.rag_enrich.code_context()` 신규 — `{candidate_id: 코드 스니펫}` **곁채널**을 만든다. `Candidate.signals`는 문자열 리스트(공통 계약)라 40줄 코드에 부적절하고 Day5 freeze로 스키마에 필드를 더할 수도 없어서, candidate를 오염시키지 않는 별도 매핑으로 나른다. 스니펫에는 **파일 안 절대 줄번호**를 붙인다(모델이 "몇 번 줄이 sink다"라고 짚을 수 있게). `model.serving.build_rerank_messages(cands, contexts=, max_context=10)`가 상위 10개 후보에만 코드를 싣는다 — 7.7 tok/s라 프롬프트 길이가 곧 지연이고 순위를 가르는 건 상위권이라서. `contexts` 없으면 예전처럼 메타만(SCA·오프라인도 재랭킹은 계속 돔).
  - **redaction egress 경계 확정**: `build_rerank_messages`가 프롬프트에 싣는 코드에 `core.redaction.redact()`를 일괄로 건다. 생산자(rag_enrich)마다 거는 대신 **경계 한 곳**에서 거는 방식 — `evidence_store.write_artifact()`가 저장 계층 한 곳에서 거는 것과 같은 패턴(cowork_rule 4절). 공유 root 서버라 대상 소스의 하드코딩 JWT/세션쿠키/password가 흘러가지 않게 하는 마지막 관문. 테스트로 JWT·password·JSESSIONID 3종이 프롬프트에서 사라지는 것을 고정.
  - `_candidate_brief` docstring("민감정보 없이 메타만")이 의도적 설계였으므로, 그 보증이 이제 `redact()`로 옮겨갔다는 것을 명시해 갱신.
- [x] **R-2. `scanners.rag_enrich.enrich()` 파이프라인 배선** — **완료**: `mcp_server/tools_analysis.py`에 `_rag_enrich(run, candidates)` 추가, `_store_scan_candidates`가 `aggregate` **앞에서** 호출한다. 이제 `rag:relevance` signal이 실제로 붙어 `priority_score`의 최대 +0.1 보너스가 살아나고(그전엔 붙이는 쪽이 안 돌아 **항상 0**이었다), 코드 스니펫이 재랭킹 훅까지 전달된다.
  - **lazy build**: `has_indexable_location()`이 먼저 `파일:줄` 후보 유무를 보고, 없으면 `CodeIndex.build()`를 **아예 호출하지 않는다** — SCA candidate는 의존성 취약점이라 전부 여기 해당하므로 `vc_run_sca`가 헛되이 소스 트리를 훑지 않는다(테스트로 `build` 미호출 고정).
  - **비파괴**: 인덱싱이 실패하면(소스 없음/권한 등) 입력 후보를 그대로 돌려준다 — RAG는 보정이지 필수 경로가 아니다.
  - **실측 확인**: `model/testdata/sample_repo`로 프롬프트를 렌더해 SQLi 취약 코드(`query = "SELECT * FROM users WHERE id = " + str(user_id)`)가 줄번호와 함께 실리는 것을 눈으로 확인.
  - **인덱스 캐시**(미착수, 선택): `CodeIndex.build`가 2806 chunks에 0.6초라 급하지 않지만, scan tool 3개 + `repair.locator`가 run당 4~5회 빌드한다. 캐시 키는 경로+mtime — 패치 적용 후 worktree는 소스가 바뀌므로 경로만으로는 stale해진다.
  - **idor sink 어휘 변별력**(미착수, 선택): `find/get/where/user/id`가 너무 일반적이라 청크의 44%가 relevance 0.67 이상을 받는다(injection 0.9%, xss 2.6%는 정상). 우선순위 신호로 쓰려면 순위가 갈려야 하므로 `authorize/permission/owner` 쪽에 가중치를 주거나 `CodeIndex._idf`로 흔한 토큰을 깎는다.
- [ ] **R-3. LLM 패치 합성 배선** — ⚠️ **2026-07-21 02:18 Discord로 분담 변경됨. 어댑터를 새로 짜지 않는다.** P3가 `repair/llm_synth.py`를 이미 작성·커밋했다(`a189c17`, `origin/security/agent`, 테스트 7/7 green). 원래 R-3 본문("235B가 controller/service/middleware 대안 `PatchCandidate`를 만든다")이 곧 P3 어댑터가 하는 일이라, 그대로 진행하면 중복이다.
  - **배경은 그대로 유효**: `template_synthesize()`는 `.java` + IDOR 소유권가드 1패턴뿐이라 XSS·SQLi·비-Java는 후보 0개 → `ValueError`. 여기가 "잘 패치한다"의 0→1 지점.
  - **안전 근거도 그대로**: 6개 게이트가 나쁜 패치를 걸러내고 3회 실패 시 HUMAN_REVIEW. judge는 누가 diff를 썼는지 보지 않는다. LLM은 "합성"에만 관여하고 "판정"엔 관여하지 않는다.
  - **P3가 이미 한 것**: `make_llm_synthesizer(client) -> synthesize_fn`, 프롬프트 조립(injection guard + secret redaction) → diff 파싱 → expected-file 필터 → `_classify_layer` 재사용 → 보수적 스코어 → `PatchCandidate`. `client=None`이면 no-op이라 기존 template 경로 무해. 비-Java(Python SQLi)에서 template=None인데 LLM 후보로 `generate_patch`가 정상 동작하는 것까지 확인됨.
  - [ ] **R-3a (P4 몫). `PatchModelClient` 구현** — 프로토콜은 `synthesize_patch(prompt: str) -> str` 하나뿐. `model.endpoints.chat_fn_from_env()` 위에 얇은 래퍼면 된다(프롬프트 문자열 → messages 1개 → diff 포함 응답). **"큰 API 두뇌"가 꽂히는 유일한 자리**(P3 표현).
  - [ ] **R-3b (P1 몫). `tools_repair.py:298` 배선** — `generate_patch(..., synthesize_fn=make_llm_synthesizer(_get_llm_client()))`. `patcher.py:240`에 kwarg이 이미 열려 있어 import + 한 줄이다.
    - **주입 패턴 합의됨(P3 제안, P1 봇이 02:26에 수용)**: 모듈 레벨 싱글턴이 아니라 **lazy init + memoize**, 실패 시 `None`. 이유 — ① import-time side effect(env 읽기·health probe)가 CI/오프라인 import를 오염시킨다, ② 235B health/fallback 체인이 import가 아니라 **use 시점**에 해소돼야 L-4의 3초 probe·티어 폴백이 제대로 돈다, ③ `client=None`이면 어댑터가 no-op이라 endpoint 불가 시 template-only로 안전하게 degrade.
  - [ ] **R-3c. R-1 컨텍스트 빌더와 정렬** — P3가 "rerank와 공유할 컨텍스트 빌더가 생기면 `build_prompt`를 거기에 맞추겠다(공유 환영)"고 했다(02:18). R-1의 `code_context()` + 경계 `redact()`가 그것이므로 P3에 알린다. 현재 `llm_synth`는 자체적으로 root_cause 파일 소스를 redaction해 싣고 있어 **중복이지만 무해**하다(`core.redaction`은 idempotent — 이미 `<redacted>`인 텍스트는 다시 매치되지 않는다).
  - **선행 조건**: `origin/security/agent`의 `a189c17`(+ `015f23c` locator rationale CWE 분기)을 pull 해야 한다. P3가 XSS/SQLi finding에도 IDOR "소유권 검사 누락" rationale이 나가던 것을 고쳤는데, 안 고쳤으면 모델이 소유권 가드를 만들어 **attack 게이트가 계속 reject**했을 문제다.
...
- [ ] **R-6. MCP 프로세스를 내부망 VM으로 이관 (host → vm → 235b)** — **2026-07-21 결정**. R-5의 답으로 **co-located 방식**을 택했다: Claude Code(랩탑, Host)가 **SSH 너머로 VM의 MCP 서버를 stdio subprocess로 실행**하고, MCP가 VM에서 도니 235B에 직접 닿는다.

  **왜 터널이 아니라 이관인가**: `.vibecutter/`(evidence.db·worktree·runs)가 `core/db.py:28`에서 **저장소 루트 기준**이라 MCP 프로세스가 도는 머신에 따라간다. 터널만 뚫으면 모델만 VM 쪽이고 evidence·Docker는 랩탑에 남아 갈라진다 — P3가 겪은 ".vibecutter는 내 로컬에만" 문제가 그대로 재현된다. 발표 데모도 결국 co-located여야 P2가 runtime metadata를 attach할 수 있다.

  **stdio 전송은 그대로다**(기획서 §6.1 / DoD "MCP 서버가 stdio로 실행, 외부 포트 없음"). SSH는 stdin/stdout을 파이프로 잇는 것뿐이라 포트를 열지 않는다 — 계약 위반이 아니다.

  - [ ] **R-6a. VM에 저장소·의존성 준비**: `git clone` + Python **3.13** venv(`.python-version`) + `pip install -r requirements.txt`. ⚠️ semgrep은 3.11~3.13만 동작(3.14 불가). vLLM용 3.10 venv와 혼동하지 말 것 — **별도 venv**다.
  - [ ] **R-6b. gitignore된 것 수동 이관**: `.env`(`VIBECUTTER_LLM_API_KEY` 등), `.env.discord`. **채널·git에 올리지 말고 scp로.**
  - [ ] **R-6c. 대상 소스 bootstrap**: `.vibecutter/targets/sources`는 201MB지만 **복사하지 않는다** — `targets/source-lock.yaml`이 repo+revision을 고정하고 있으므로 VM에서 `python -m runtime.source_bootstrap <target_id>`로 받는다(P2가 camp1에서 이미 c2-04에 쓴 방법).
  - [ ] **R-6d. Docker 대상앱을 VM에서 기동**: `policies/scope.yaml`의 allowlist가 `127.0.0.1:140xx`인데, 이건 **MCP 프로세스 기준 localhost**다. MCP가 VM에서 돌면 대상 컨테이너도 VM에 있어야 정책 검사를 통과한다. scope.yaml은 **수정 불필요**(주소가 그대로 유효).
  - [ ] **R-6e. Host 설정 전환**: `~/.claude.json`의 `vibecutter` 항목을 `command: ssh`로 바꾼다.
    ```json
    { "type": "stdio", "command": "ssh",
      "args": ["-T", "<vm-host>", "/path/Vutter/.venv/bin/python", "/path/Vutter/mcp_server/server.py"] }
    ```
    ⚠️ **stdout 순수성**: MCP는 stdout이 JSON-RPC 전용이어야 한다(DoD 항목). SSH 배너/MOTD/셸 rc가 stdout에 한 줄이라도 쓰면 **프로토콜이 깨진다**. `-T`(pty 없음) + `~/.hushlogin` 또는 배너 비활성 + 비대화형 셸에서 `echo` 없는 rc를 확인할 것. 검증은 `vc_ping` → `"pong"`.
    ⚠️ **SSH 키 인증 필수** — 비대화형이라 비밀번호 프롬프트가 뜨면 그대로 멈춘다. (P2가 07-20 13:43에 camp1 publickey 거부로 막힌 전례)
  - [ ] **R-6f. 개발 워크플로 정리**: 코드는 랩탑에서 편집하는데 MCP는 VM의 사본을 실행한다 → **동기화 규약이 필요하다.** git push/pull을 기본으로 하고(커밋은 사람이 — 팀 규칙), 급할 때만 rsync. 이 갭이 이관의 가장 큰 실무 비용이니 팀에 명시한다.
  - [ ] **R-6g. 이관 후 실검증**: `vc_ping` → `python -m model.endpoints`가 VM에서 `[UP]`을 보이는지 → R-3a `PatchModelClient` 실동작 → c1-05 fresh run.
  - **랩탑에 남는 것**: 우리 `.vibecutter/evidence.db`(run 302/finding 158/candidate 224)는 대부분 유닛테스트 궤적이라 **옮기지 않는다**(P3가 확인한 하이진 이슈 — 실앱 gold는 `run-897ad65c686f` 등 P3 로컬에 있다). VM은 fresh DB로 시작한다.

- [ ] **R-5. [팀 결정] endpoint 도달성 매트릭스 → loop 실행 위치 확정** — ⚠️ **Phase LLM L-2가 형태를 바꿔 되살아났다.** 외부 API 전환으로 "GPU 서버 위에서 loop을 돌릴지" 문제가 사라졌다고 L-4에 적었는데, **틀렸다**. R-6으로 답을 냈다(VM co-located). 아래 매트릭스는 그 근거다:

  | 위치 | `192.168.0.226:8080` | 근거 |
  | --- | --- | --- |
  | 내부망 VM | ✅ 접속 확인 | 2026-07-21 사용자 확인 |
  | P1 랩탑 | ❌ timeout | `python -m model.endpoints` |
  | P2 로컬 | ❌ timeout | Discord 02:25 |
  | camp1~3 GPU 서버 | ❓ **미확인** | — |

  `172.10.7.246`(외부망)은 P2 로컬에서도 timeout(02:12)이라 도달성 근거가 아직 없다.

  - **왜 중요한가**: L-4의 3초 `/health` probe는 못 닿으면 **조용히 휴리스틱으로 degrade**한다. 즉 지금 랩탑에서 돌리는 모든 run은 LLM 없이 돌고 있고, 그 사실이 로그로 크게 드러나지 않는다. **R-4 ablation 표본이 오염된다** — P4가 이미 같은 지적을 했다("health/readiness가 false였던 run은 base-vs-full 비교 표본에서 빼야 한다", 07-20 13:08).
  - **결정할 것**: ① camp1~3에서 235B에 닿는가 → 닿으면 loop을 GPU 서버에서 돌린다(모델도 evidence도 co-located). ② 안 닿으면 SSH 터널(7B가 이미 쓰던 방식) 또는 방화벽 allowlist. ③ 랩탑 개발은 `VIBECUTTER_LLM_DISABLE=1`로 명시적 휴리스틱을 기본으로 삼는다(조용한 degrade보다 낫다).
  - [ ] **R-5a. run 메타데이터에 LLM 사용 여부 기록** — 어느 tier가 응답했는지(또는 폴백했는지)를 trajectory/report에 남겨 ablation 표본을 가려낼 수 있게 한다. `make_chained_chat_fn(on_fallback=)` 훅이 이미 있으므로 그걸 쓴다.

**명세 확인 결과(기록용)**: 기획서 §3 "모델·소스·취약점 데이터가 외부로 나가지 않는다"는 **조직 경계**를 뜻하며, 내부망 VM의 vLLM은 7B(camp1~3 + SSH 터널)와 같은 구조라 Local-first 원칙에 어긋나지 않는다. 코드를 모델에 주는 것도 §8.1(Embedding "source chunk retrieval")·§8.2(Phase 0 "Tool RAG")·부록 trajectory 스키마(`"observation": ["controller code", ...]`)가 이미 전제한다. 단 `172.10.7.246`은 사설 대역(`172.16.0.0/12`)이 아니라 공인 IP 공간이라 외부 도달성 확인이 필요하다(R-5).

**용어 정정 필요**: P2가 Discord에서 이 모델을 계속 **"72B"**로 부르고 있다(01:57 ×3). 실제는 **qwen3-235b**다. P2의 D7 runtime plan·평가 표 필드에 그대로 들어가 있을 수 있으니 정정한다.
...
- [x] ~~**P2**: `172.10.7.246` 외부 도달성 확인 요청~~ **✅ 회신 옴(02:12/02:25)**: P2 로컬에서 `172.10.7.246`·`192.168.0.226` **둘 다 timeout**. "서버 자체 down인지 네트워크/방화벽 차단인지 구분 안 됨". → R-5로 승격.
- [x] ~~**P3**: R-3 배선 방식 이견 확인~~ **✅ 회신 옴(02:18)**: 이견 없음 + **어댑터를 이미 만들어 커밋함**(`a189c17`). 분담 확정 — P4=`PatchModelClient`, P1=배선, P3=어댑터(완료). R-3 본문을 이에 맞게 수정함.
- [x] **보고서 담당**: R-0 RQ3 재정의 — **회의 확정 완료**.
- [ ] **P2 (신규)**: ① **용어 정정** — "72B"가 아니라 **qwen3-235b**. D7 runtime plan/평가 표 필드 확인 요청. ② **R-5 도달성 매트릭스** — camp1~3 GPU 서버에서 `192.168.0.226:8080/health`에 닿는지 확인 요청(loop 실행 위치가 여기 걸림). ③ Juice Shop regression 계약 A/B는 P3 소관이라 그쪽 회신 대기.
- [ ] **P3 (신규)**: ① **R-1 공유 컨텍스트 빌더 준비됨** — `scanners.rag_enrich.code_context()`(`{candidate_id: 줄번호 붙은 스니펫}`) + `model.serving.build_rerank_messages`의 경계 `redact()`. 02:18에 "생기면 `build_prompt`를 맞추겠다"고 한 그것이다. ② `llm_synth`의 자체 redaction은 **그대로 둬도 무해**(idempotent) — 급히 걷어낼 필요 없다. ③ Juice Shop regression 계약 A(`npm test`) vs B(image smoke) 회신이 P2 대기 중.
- [ ] **P4 (신규)**: R-3a `PatchModelClient` 구현이 P4 몫으로 확정됐다. `model.endpoints.chat_fn_from_env()` 래퍼면 되며, **R-5가 풀리기 전엔 실동작 검증이 불가**(랩탑에서 endpoint 미도달)하다는 것 공유.
