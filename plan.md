...
- [~] **0-4. 회귀 테스트**: (a)(b)(c) xss/injection verify tool 배선 테스트는 **0-1에서 완료**(`VcVerifyInjectionXssToolTests`). 남은 것: `test_write_oracle_wiring.py`의 `CheckAttackAutoDispatchTests`에 xss/injection candidate 라우팅 케이스 추가 — **0-2(check_attack vuln_class 라우팅) 구현 후** 함께 처리.
...
- [ ] **1B-1. `Candidate.origin_candidate_id` lineage 필드 추가** (D5-P2.md 계약 ②, additive):
  - [ ] `contracts/schemas.py`의 `Candidate`에 `origin_candidate_id: Optional[str] = None` 추가. worker Run으로 materialize할 때 원본 scan candidate ID를 여기 보존한다(scan candidate 자체의 `run_id`는 절대 덮어쓰지 않음).
  - [ ] `core/evidence_store.py`의 `CandidateRow`에도 동일 컬럼 동기화.
  - [ ] **SQLite migration 주의**(D5-P2.md가 P1에 명시 요청): `SQLModel.create_all()`은 기존 테이블에 컬럼을 추가하지 않는다. `.vibecutter/evidence.db`가 이미 있으면 `no such column: candidate.origin_candidate_id`가 난다 — 로컬은 gitignored라 삭제 후 재생성으로 처리하고, **팀 전체에 "이 필드 추가 후 각자 로컬 DB 삭제 필요"를 공지**(섹션 5 Day2에서 이미 겪은 패턴). 실 데이터가 쌓였으면 `ALTER TABLE candidate ADD COLUMN origin_candidate_id` 최소 migration 스니펫 제공.
  - [ ] 테스트: worker Candidate가 origin_candidate_id를 담고 round-trip, 기존 scan Candidate는 None으로 정상 동작.
- [ ] **1B-2. worker Run materialize 헬퍼** (신규 `core/orchestrator.py` 또는 `core/planner.py` 확장):
  - [ ] `materialize_worker_run(scan_run, scan_candidate) -> tuple[Run, Candidate]`: 새 worker Run(`status=CANDIDATE_SCAN`, 같은 target_id)을 만들고, scan candidate를 복제해 `run_id=worker_run.id` + `origin_candidate_id=scan_candidate.id`로 저장한다. 원본 scan candidate/scan Run은 불변.
  - [ ] scan Run은 `CANDIDATE_SCAN`에서 종료(계약 ①) — 더 이상 전이시키지 않는다. verify/localize/patch/validate는 전부 worker Run에서만 일어난다(계약 ③).
  - [ ] 테스트: materialize 후 worker Run이 독립 상태 진행 가능, scan Run은 CANDIDATE_SCAN 고정, lineage 보존.
- [ ] **1B-3. batch 오케스트레이션 함수** (`core/orchestrator.py`): `run_target_audit(target_id)` — Day5 데모의 "명령 한 줄"에 해당하는 단일 진입점.
  - [ ] **batch 시작 전**: `service.sweep_stale_run_overlays(target_id, active_run_ids=<현재 실행 중 worker run id들>, approved=True)` 호출(D5-P2.md P2 호출 조건). 최초엔 active 없음.
  - [ ] scan Run 1개 생성 → `vc_scan_access_control`(+필요 시 `vc_run_sast`/`vc_run_sca`)로 후보 수집 → `CANDIDATE_SCAN`에서 종료.
  - [ ] 각 candidate에 대해 **순차로**(계약 ④ 고정 포트) worker Run을 materialize하고 verify→(verified면)localize→generate_patch→apply→build_and_test→replay_attack→validate_regression 파이프라인 실행.
  - [ ] **overlay를 만든 worker Run 종료 `finally`**: `service.reset_run(target_id, worker_run.id, approved=True)`. **단, overlay 생성 여부를 기준으로만 호출**(D5-P2.md 리스크: `reset_run`은 run-scoped patched overlay용이라 scan/verify-only worker엔 무조건 호출 금지) — patch가 apply돼 overlay가 생긴 worker Run에만 건다. reset 실패해도 P2가 artifact/worktree를 보존하므로 예외로 죽이지 않고 로깅만.
  - [ ] 테스트: `_service()`/tool을 mock으로 대체해 (a) 시작 시 sweep 1회, (b) candidate마다 worker Run 1개 생성, (c) overlay 만든 worker Run만 `finally`에서 reset_run 호출, (d) verify-only(rejected) worker Run은 reset_run 미호출, (e) 순차 실행 순서 확인.
- [ ] **1B-4. `audit_local_target` 프롬프트 갱신** (`mcp_server/prompts.py`): step 4/5를 "candidate마다 새 worker Run을 만들어 verify→patch loop를 독립 실행"으로 갱신. scan Run과 worker Run 구분을 Host에게 명확히 안내. (Phase 2-6과 통합 — 아래에서 제거하고 여기로 흡수.)
- [ ] **1B-5. c1-05로 batch 라이브 리허설** (여유 시): `run_target_audit("26s-w1-c1-05")`를 실 target으로 한 바퀴 — sweep→scan→worker Run→closed-loop→reset_run이 실제로 도는지. P3가 이미 수동으로 완주한 경로를 P1 오케스트레이터로 자동 재현. 환경 제약(로컬 source clone)으로 P1 세션에서 안 되면 P3 환경 실측 요청.
