...
git stderr나 토큰이 섞여 나올 경우까지 포함한다.
...
- **patch diff export(`vc_export_patch`가 쓰는 `.patch` 파일)는 의도적으로 redaction하지
  않는다.** diff는 `git apply`로 실제 코드에 적용돼야 하는 바이트 정확한 산출물이라,
  본문에 `<redacted>`를 끼워 넣으면 hunk 길이가 틀어져 diff가 깨지거나 사용자가 잘못된
  코드를 적용하게 된다. 리포트(사람이 읽음)와 export(기계가 적용함)는 요구사항이 달라
  같은 처리를 쓸 수 없다 — 별도 접근이 필요하며 아직 미해결이다.
- **container/process 로그 redaction은 아직 구현되지 않았다.**
- **SARIF export(`eval/report_export.py::render_sarif`)는 현재 `redact()`를 한 번도
  호출하지 않는다** — `finding.impact` 같은 자유 텍스트가 그대로 SARIF `message.text`에
  들어갈 수 있다. HTML 리포트(같은 데이터 소스)는 이미 redaction되지만 SARIF는 아직이다.

이 세 항목은 `REMAINING_PLAN.md`에 열린 작업으로 추적 중이다.
...
- **patch diff / container log redaction**: 6절에 적은 그대로, 아직 미해결이다.
- **SARIF redaction**: 6절에 적은 그대로, 아직 미해결이다.
