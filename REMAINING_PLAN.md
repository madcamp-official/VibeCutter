...
- [x] **I4. 콘텐츠 유사도 기반 차등 보강** — `verifiers/injection.py:injection_oracle` (완료, `7ee48b0`)
  - **무엇→한 일**: 길이-델타 + 상태코드에 **콘텐츠 발산 신호(2b)**를 추가. 참(모든 행)과
    거짓(빈 결과)의 byte 길이가 우연히 비슷해 델타가 놓치는 recall 구멍을 닫는다 — 두 본문의
    구조 유사도(`_ratio`, SequenceMatcher, 4000자 상한)가 낮으면 결과셋 열/닫힘으로 판정.
  - **precision 불변식 유지(엄격한 Pareto)**: sim ≤ 0.4 **and** benign 2-sample 유사도 바닥 대비
    margin ≥ 0.4일 때만 발화 → 한 글자 에코(sim≈1.0)·살균 앱(두 무효값이 같은 '없음' 페이지)·
    노이즈 엔드포인트·상태 불안정·짧은 본문은 절대 안 걸린다. 파괴적 payload 없음(불변).
  - **완료 판정**: self-check **8/8**(발산 탐지 + 발산 precision 추가), 하드닝 회귀 테스트 **7종**
    (`InjectionContentDivergenceTests`) — 발산 탐지 1 + precision 4 + 노이즈/상태/짧은본문 억제 3.

**③ 패치 성공률**

- [~] **I5. injection 패치 6게이트 통과율 측정·개선** — `repair/locator.py`(sink 힌트)
  - **(개선) 완료(`980eb82`)**: SAST 폴백의 sink 파일 선택을 **cross-file 인지**로 보강. route
    매핑 실패 시 기존엔 `sorted(sast)[0]`(알파벳)으로 골라 쿼리 조립·실행이 다른 파일이면 실행부가
    아닌 파일을 짚을 수 있었다. sink형(XSS/SQLi)에 한해 SAST 보고 순서의 첫 sink(`source_symbols[0]`)를
    채택해 route 경로의 sink-우선(`sinks[0]`)과 일치시키고, 후보가 여럿이면 목록+실행부 우선 안내를
    rationale에 남긴다. IDOR·단일 파일은 무회귀. 테스트 3종(`LocalizeSastFallbackTests`).
  - **(측정) 런타임 대기**: 여러 injection 케이스에서 `PATCH_APPLIED → FIXED` 통과율 기록은 235B
    패치 실주행이 필요 — 오케스트레이터/모델 실행(4.4 측정과 연결). P3 코드 쪽 보완은 위로 소진.
...
- [~] **M1. Injection·XSS를 ablation/eval 표본에 포함** — `eval/priority_ablation.py`, `eval/compare.py`
  **(2026-07-22 P4 갱신)** — **클래스별 breakdown 하네스 완료**: `eval/priority_ablation.compare_by_class`
  (injection·xss·idor별 MRR/first_true_rank, 테스트 7/7). 전처리 `eval/reflect_runs.py`(235B/휴리스틱
  판정 + P2 runtime_metadata 대조, 7/7)도 완료. **남은 건 벤치 16앱 소스 확보 후 실주행·결과
  아티팩트뿐**(P2 소스 0→7/16 부분). 하네스·지표는 준비 완료.
...
- [~] **M2. 클래스별 패치 성공률(6게이트 통과율) 집계** — 별도 하네스
  **(2026-07-22 P4 갱신)** — **하네스 완료**: `eval/patch_success.py`(idor/injection/xss별 verified→FIXED
  성공률 + 6게이트 통과율, `GATES==core.judge._GATE_FIELDS` 드리프트 테스트, 5/5). **실집계는 데모
  closed-loop의 Validation(verdict/게이트) 필요**(P3 juice/데모 run).
  - **완료 판정**: idor/injection/xss 각각 `verified→FIXED` 성공률 표가 발표 자료에 들어간다.

### 4.4 (참고) SAST 규칙 커버리지 — 담당 P4

- [x] **주의**: 위 surface(정적 프리필터) 확장과 별개로, `scanners/sast`의 semgrep 규칙도
  injection/xss sink를 충분히 커버하는지 점검한다. surface와 SAST는 **다른 경로로 후보를 낸다**
  (`aggregate`에서 합쳐짐). 둘 중 하나만 커버하면 놓친다.
  - **(2026-07-22 P4 점검 완료)** `eval/results/sast_coverage_audit.md`. 판정: **SQLi(CWE-89)/XSS(CWE-79)
    데모 sink는 `p/sql-injection`·`p/xss`로 커버됨.** 갭 = NoSQL(CWE-943)·LDAP(CWE-90) 전용 룰셋 부재,
    IDOR SAST 구조적 미탐 → 둘 다 surface 프리필터 + 동적 verify가 다른 경로로 보완(파이프라인 전체 커버).
...
- [~] **[P4]** 4.3 **M1/M2** — heuristic vs rag-llm 클래스별 verified precision·순위 개선·패치 성공률.
  **하네스 완료**(`compare_by_class`·`patch_success.py`), 실집계는 P2 벤치소스(M1)·P3 Validation(M2) 대기.
